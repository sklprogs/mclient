...
class Portion:
    ...
    def set_wforms(self):
        ...
        f = '[MClient] converters.mdic_shared.Portion.set_wforms'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def set_json(self):
        ''' #NOTE: Edit Poses.start_pattern and Poses.end_pattern upon changing
            JSON structure.
        '''
        f = '[MClient] converters.mdic_shared.Portion.set_json'
        if not self.Success:
            rep.cancel(f)
            return
        ms.STOP = True
    ...
    def dump_json(self):
        f = '[MClient] converters.mdic_shared.Portion.dump_json'
        if not self.Success:
            rep.cancel(f)
            return
        try:
    ...
    def _debug_wforms_fragms(self):
        ...
        f = '[MClient] converters.mdic_shared.Portion._debug_wforms_fragms'
    ...
    def set_fragms(self):
        f = '[MClient] converters.mdic_shared.Portion.set_fragms'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def _compress(self, data):
        ...
        '''
        f = '[MClient] converters.mdic_shared.Portion._compress'
        try:
    ...
    def set_body(self):
        f = '[MClient] converters.mdic_shared.Portion.set_body'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def save_body(self):
        f = '[MClient] converters.mdic_shared.Portion.save_body'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def set_sources(self):
        ...
        f = '[MClient] converters.mdic_shared.Portion.set_sources'
        if not self.Success:
            rep.cancel(f)
            return
...
class Block:
    ...
    def run(self):
        f = '[MClient] converters.mdic_shared.Block.run'
        if not self.block:
...
class Poses:
    ...
    def set_start(self):
        f = '[MClient] converters.mdic_shared.Poses.set_start'
        if not self.Success:
            rep.cancel(f)
            return
        pos = 0
    ...
    def set_end(self):
        f = '[MClient] converters.mdic_shared.Poses.set_end'
        if not self.Success:
            rep.cancel(f)
            return
        pos = 0
    ...
    def check(self):
        f = '[MClient] converters.mdic_shared.Poses.check'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def set_fragms(self):
        f = '[MClient] converters.mdic_shared.Poses.set_fragms'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def _debug_poses(self):
        f = '[MClient] converters.mdic_shared.Poses._debug_poses'
        print(f + ':')
        print('start:', self.start)
        print('end:', self.end)
    
    def _debug_fragms(self):
        f = '[MClient] converters.mdic_shared.Poses._debug_fragms'
        print(f + ':')
        for i in range(len(self.fragms)):
            print(i + 1, ':')
            print('"' + self.fragms[i] + '"')
    ...
    def _debug_dics(self):
        f = '[MClient] converters.mdic_shared.Poses._debug_dics'
        print(f + ':')
        for i in range(len(self.fragms)):
    ...
    def debug(self):
        f = '[MClient] converters.mdic_shared.Poses.debug'
        if not self.Success:
            rep.cancel(f)
            return
        self._debug_poses()
    ...
    def free_memory(self):
        f = '[MClient] converters.mdic_shared.Poses.free_memory'
        if not self.Success:
            rep.cancel(f)
            return
        self.start = []
...
class Index:
    ...
    def _save(self, abbr, bytes_):
        f = '[MClient] converters.mdic_shared.Index._save'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def save(self):
        f = '[MClient] converters.mdic_shared.Index.save'
        if not self.Success:
            rep.cancel(f)
            return
