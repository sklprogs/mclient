...
class Speech:
    ...
    def set_values(self):
        ...
        self.code = ''
        self.dic = {}
        self.file_ptrn = PDIR.add('..', 'resources', 'sources', 'multitrancom'
                                 ,'speech', '{}.json')
    
    def set_dic(self):
        f = '[MClient] sources.multitrancom.speech.Speech.set_dic'
        if not self.Success:
            rep.cancel(f)
            return
        try:
    ...
    def load(self):
        f = '[MClient] sources.multitrancom.speech.Speech.load'
        if not self.Success:
            rep.cancel(f)
            return
