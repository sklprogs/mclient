...
class Dic:
    ...
    def check(self):
        f = '[MClient] sources.multitrancom.utils.subjects.Dic.check'
    ...
    def set_subjects(self):
        f = '[MClient] sources.multitrancom.utils.subjects.Dic.set_subjects'
        if not self.Success:
            rep.cancel(f)
            return
        i = 1
    ...
    def add(self):
        f = '[MClient] sources.multitrancom.utils.subjects.Dic.add'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def sort(self):
        f = '[MClient] sources.multitrancom.utils.subjects.Dic.sort'
        if not self.Success:
            rep.cancel(f)
            return
        subjects = {}
...
class Loop:
    ...
    def __init__(self):
        ...
        ''' Copy "var subjects" and "var MajorToMinor" sections from
            ...
            zh: 17.
        '''
        self.filew = PDIR.add('..', '..', '..', '..', 'resources', 'sources'
                             ,'multitrancom', 'subjects','subjects.json')
    
    def input(self):
        f = '[MClient] sources.multitrancom.utils.subjects.Loop.input'
        if not self.Success:
            rep.cancel(f)
            return
...
#            return
    ...
    def _convert(self, string):
        f = '[MClient] sources.multitrancom.utils.subjects.Loop._convert'
        try:
    ...
    def set_lists(self):
        f = '[MClient] sources.multitrancom.utils.subjects.Loop.set_lists'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def add(self):
        f = '[MClient] sources.multitrancom.utils.subjects.Loop.add'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def save(self):
        f = '[MClient] sources.multitrancom.utils.subjects.Loop.save'
        if not self.Success:
            rep.cancel(f)
            return
...
class Commands:
    ...
    def get_string(self, dic):
        f = '[MClient] sources.multitrancom.utils.subjects.Commands.get_string'
        try:
