...
class CleanUp:
    ...
    def delete_langs(self):
        ...
        f = '[MClient] sources.multitrancom.cleanup.CleanUp.delete_langs'
    ...
    def fix_wform_com(self):
        # EN-RU, hello
        f = '[MClient] sources.multitrancom.cleanup.CleanUp.fix_wform_com'
    ...
    def fix_wforms(self):
        f = '[MClient] sources.multitrancom.cleanup.CleanUp.fix_wforms'
    ...
    def fix_href(self):
        ...
        '''
        f = '[MClient] sources.multitrancom.cleanup.CleanUp.fix_href'
        if not self.text:
            rep.empty(f)
            return
        count = 0
    ...
    def run(self):
        f = '[MClient] sources.multitrancom.cleanup.CleanUp.run'
        if not self.text:
            rep.empty(f)
            return ''
