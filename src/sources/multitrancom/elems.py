...
class Trash:
    ...
    def report(self):
        f = '[MClient] sources.multitrancom.elems.Trash.report'
    ...
    def delete(self):
        f = '[MClient] sources.multitrancom.elems.Trash.delete'
    ...
    def remove_stresses(self):
        ...
        f = '[MClient] sources.multitrancom.elems.Trash.remove_stresses'
        old_len = len(self.blocks)
...
class Thesaurus:
    ...
    def add(self):
        f = '[MClient] sources.multitrancom.elems.Thesaurus.add'
        if self.no is None:
            rep.lazy(f)
            return
        count = 0
    ...
    def delete(self):
        f = '[MClient] sources.multitrancom.elems.Thesaurus.delete'
        if self.no is None:
            rep.lazy(f)
            return
        try:
...
class Suggestions:
    ...
    def has(self):
        f = '[MClient] sources.multitrancom.elems.Suggestions.has'
        if not self.Success:
            rep.lazy(f)
            return
        for block in self.blocks:
    ...
    def set_types(self):
        f = '[MClient] sources.multitrancom.elems.Suggestions.set_types'
        if not self.Success:
            rep.lazy(f)
            return
        count = 0
    ...
    def set_head(self):
        f = '[MClient] sources.multitrancom.elems.Suggestions.set_head'
        if not self.Success:
            rep.lazy(f)
            return
    ...
    def set_tail(self):
        ...
        f = '[MClient] sources.multitrancom.elems.Suggestions.set_tail'
        if not self.Success:
            rep.lazy(f)
            return
        i = len(self.blocks) - 1
    ...
    def cut(self):
        f = '[MClient] sources.multitrancom.elems.Suggestions.cut'
        if not self.Success:
            rep.lazy(f)
            return
        old_len = len(self.blocks)
    ...
    def debug(self):
        # Orphaned
        f = '[MClient] sources.multitrancom.elems.Suggestions.debug'
        if not self.Success:
            rep.lazy(f)
            return
        for i in range(len(self.blocks)):
    ...
    def delete_semi(self):
        ...
        f = '[MClient] sources.multitrancom.elems.Suggestions.delete_semi'
        if not self.Success:
            rep.lazy(f)
            return
        old_len = len(self.blocks)
...
class Elems:
    ...
    def run_phcount(self):
        f = '[MClient] sources.multitrancom.elems.Elems.run_phcount'
        count = 0
        i = 1
        while i < len(self.blocks):
    ...
    def delete_semi(self):
        f = '[MClient] sources.multitrancom.elems.Elems.delete_semi'
        old_len = len(self.blocks)
        self.blocks = [block for block in self.blocks if block.text != '; ']
        rep.matches(f, old_len - len(self.blocks))
    
    def separate_fixed(self):
        f = '[MClient] sources.multitrancom.elems.Elems.separate_fixed'
        count = 0
        i = 1
        while i < len(self.blocks):
    ...
    def separate_sp_transc(self):
        ...
        '''
        f = '[MClient] sources.multitrancom.elems.Elems.separate_sp_transc'
        count = 0
        i = 1
        while i < len(self.blocks):
    ...
    def set_transc(self):
        f = '[MClient] sources.multitrancom.elems.Elems.set_transc'
        count = 0
        for block in self.blocks:
    ...
    def renumber_by_type(self):
        f = '[MClient] sources.multitrancom.elems.Elems.renumber_by_type'
        count = 0
        i = 1
        while i < len(self.blocks):
    ...
    def run_com_sp_com(self):
        ''' '<em>' tag usually denoting a speech part can actually be
            a comment: EN-RU: bit.
        '''
        f = '[MClient] sources.multitrancom.elems.Elems.run_com_sp_com'
        count = 0
        i = 2
        while i < len(self.blocks):
    ...
    def unite_comments(self):
        ...
        f = '[MClient] sources.multitrancom.elems.Elems.unite_comments'
        count = 0
        i = len(self.blocks) - 2
        while i >= 0:
