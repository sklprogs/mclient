...
''' About this module:
    ...
    - Use sources.multitrancom.utils.subjects to extract and process these
      subjects.
...
class ByLangs:
    ...
    def __init__(self):
        self.Success = True
        self.psubjects = PDIR.add('..', 'resources', 'sources', 'multitrancom'
                                 ,'subjects', 'subjects.json')
        self.pschema = PDIR.add('..', 'resources', 'sources', 'multitrancom'
                               ,'subjects', 'schema.json')
    
    def set_files(self):
        f = '[MClient] sources.multitrancom.subjects.ByLangs.set_files'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def load(self):
        f = '[MClient] sources.multitrancom.subjects.ByLangs.load'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def get(self):
        f = '[MClient] sources.multitrancom.subjects.ByLangs.get'
        if not self.Success:
            rep.cancel(f)
            return {}
        try:
    ...
    def dump(self):
        f = '[MClient] sources.multitrancom.subjects.ByLangs.dump'
        if not self.Success:
            rep.cancel(f)
            return ''
