...
class Xor:
    ...
    def check(self):
        f = '[MClient] sources.multitrandem.utils.Xor.check'
    ...
    def report(self):
        f = '[MClient] sources.multitrandem.utils.Xor.report'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def analyze(self):
        f = '[MClient] sources.multitrandem.utils.Xor.analyze'
        if not self.Success:
            rep.cancel(f)
            return
...
class Tests:
    ...
    def gen_patterns(self):
        f = '[MClient] sources.multitrandem.utils.Tests.gen_patterns'
    ...
    def analyze_xor(self):
        f = '[MClient] sources.multitrandem.utils.Tests.analyze_xor'
        bytes1 = b'Bullshit!'
        bytes2 = b'-fcivqx\x89<'
    ...
    def get_patch(self):
        #f = '[MClient] sources.multitrandem.utils.Tests.get_patch'
    ...
    def corrupt(self):
        f = '[MClient] sources.multitrandem.utils.Tests.corrupt'
    ...
    def analyze_dumps(self):
        f = '[MClient] sources.multitrandem.utils.Tests.analyze_dumps'
    ...
    def close_dumps(self):
        f = '[MClient] sources.multitrandem.utils.Tests.close_dumps'
    ...
    def compare_bytes(self, maxlen=10):
        f = '[MClient] sources.multitrandem.utils.Tests.compare_bytes'
    ...
    def get_shared_dumps(self):
        f = '[MClient] sources.multitrandem.utils.Tests.get_shared_dumps'
        pos1 = 0
        pos2 = 16380
...
class Parser(gt.Binary):
    ...
    def parsel23(self, chunk):
        f = '[MClient] sources.multitrandem.utils.Parser.parsel23'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def chunk3(self, chunk):
        f = '[MClient] sources.multitrandem.utils.Parser.chunk3'
        if not self.Success:
            rep.cancel(f)
            return
        if len(chunk) % 3 == 0:
    ...
    def parse_glue(self):
        f = '[MClient] sources.multitrandem.utils.Parser.parse_glue'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def parse_article(self):
        f = '[MClient] sources.multitrandem.utils.Parser.parse_article'
        if not self.Success:
            rep.cancel(f)
            return
        for chunk in self.chunks1:
    ...
    def debug(self):
        f = '[MClient] sources.multitrandem.utils.Parser.debug'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def get_chunk7(self, chunk):
        f = '[MClient] sources.multitrandem.utils.Parser.chunk7'
    ...
    def parsel1(self):
        f = '[MClient] sources.multitrandem.utils.Parser.parsel1'
        if not self.Success:
            rep.cancel(f)
            return
        for chunk in self.chunks1:
    ...
    def parse_stem(self):
        f = '[MClient] sources.multitrandem.utils.Parser.parse_stem'
        if not self.Success:
            rep.cancel(f)
            return
        self.parsel1()
    ...
    def parse(self):
        f = '[MClient] sources.multitrandem.utils.Parser.parse'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def run_reader(self, pos1, pos2):
        f = '[MClient] sources.multitrandem.utils.Parser.reader'
        if not self.Success:
            rep.cancel(f)
            return
...
class Navigate(gt.Binary):
    ...
    def find_prev(self):
        f = '[MClient] sources.multitrandem.utils.Navigate.find_prev'
        if not self.Success:
            rep.cancel(f)
            return
        if not self.coded:
    ...
    def find_next(self):
        f = '[MClient] sources.multitrandem.utils.Navigate.find_next'
        if not self.Success:
            rep.cancel(f)
            return
        if not self.coded:
    ...
    def _print_found(self):
        f = '[MClient] sources.multitrandem.utils.Navigate._print_found'
        if self.spos is None:
            mes = _('No matches!')
            Message(f, mes).show_info()
            return
    ...
    def _find(self):
        f = '[MClient] sources.multitrandem.utils.Navigate._find'
    ...
    def find_text(self):
        f = '[MClient] sources.multitrandem.utils.Navigate.find_text'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def find_bytes(self):
        f = '[MClient] sources.multitrandem.utils.Navigate.find_bytes'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def find_nav(self):
        f = '[MClient] sources.multitrandem.utils.Navigate.find_nav'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def dump(self):
        f = '[MClient] sources.multitrandem.utils.Navigate.dump'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def go_end(self):
        f = '[MClient] sources.multitrandem.utils.Navigate.go_end'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def go_start(self):
        f = '[MClient] sources.multitrandem.utils.Navigate.go_start'
        if not self.Success:
            rep.cancel(f)
            return
        self.pos = 0
    ...
    def clear(self):
        f = '[MClient] sources.multitrandem.utils.Navigate.clear'
        if not self.Success:
            rep.cancel(f)
            return
        try:
    ...
    def set_pos(self):
        f = '[MClient] sources.multitrandem.utils.Navigate.set_pos'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def go_page_down(self):
        f = '[MClient] sources.multitrandem.utils.Navigate.go_page_down'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def go_page_up(self):
        f = '[MClient] sources.multitrandem.utils.Navigate.go_page_up'
        if not self.Success:
            rep.cancel(f)
            return
        self.pos -= self.buffer
    ...
    def load(self):
        f = '[MClient] sources.multitrandem.utils.Navigate.load'
        if not self.Success:
            rep.cancel(f)
            return
        start = self.pos
    ...
    def set_buffer(self):
        f = '[MClient] sources.multitrandem.utils.Navigate.set_buffer'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def show_help(self):
        f = '[MClient] sources.multitrandem.utils.Navigate.show_help'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def show_menu(self, command=''):
        f = '[MClient] sources.multitrandem.utils.Navigate.show_menu'
        if not self.Success:
            rep.cancel(f)
            return
        if not command:
    ...
    def quit(self):
        f = '[MClient] sources.multitrandem.utils.Navigate.quit'
        if not self.Success:
            rep.cancel(f)
            return
        self.close()
    ...
    def print(self):
        f = '[MClient] sources.multitrandem.utils.Navigate.print'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def report(self):
        f = '[MClient] sources.multitrandem.utils.Navigate.report'
        if not self.Success:
            rep.cancel(f)
            return
        self.clear()
...
class Commands:
    ...
    def get_patch(self, file, pattern, pos, add_pos=20, sympos=0):
        f = '[MClient] sources.multitrandem.utils.Commands.get_patch'
    ...
    def corrupt(self, filew, pos, subst=b'\x00'):
        f = '[MClient] sources.multitrandem.utils.Commands.corrupt'
    ...
    def input_int(self, mes=''):
        f = '[MClient] sources.multitrandem.utils.Commands.input_int'
        if not mes:
...
class CompareBinaries:
    ...
    def set_files(self):
        f = '[MClient] sources.multitrandem.utils.CompareBinaries.set_files'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def dump(self):
        f = '[MClient] sources.multitrandem.utils.CompareBinaries.dump'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def go_end(self):
        f = '[MClient] sources.multitrandem.utils.CompareBinaries.go_end'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def go_start(self):
        f = '[MClient] sources.multitrandem.utils.CompareBinaries.go_start'
        if not self.Success:
            rep.cancel(f)
            return
        self.pos = 0
    ...
    def clear(self):
        f = '[MClient] sources.multitrandem.utils.CompareBinaries.clear'
        if not self.Success:
            rep.cancel(f)
            return
        try:
    ...
    def set_pos(self):
        f = '[MClient] sources.multitrandem.utils.CompareBinaries.set_pos'
        if not self.Success:
            rep.cancel(f)
            return
        mes = _('Enter a position to go or press Return to keep the current one ({}): ')
    ...
    def go_prev(self):
        f = '[MClient] sources.multitrandem.utils.CompareBinaries.go_prev'
        if not self.Success:
            rep.cancel(f)
            return
        while True:
    ...
    def go_next(self):
        f = '[MClient] sources.multitrandem.utils.CompareBinaries.go_next'
        if not self.Success:
            rep.cancel(f)
            return
        while True:
    ...
    def go_page_down(self):
        f = '[MClient] sources.multitrandem.utils.CompareBinaries.go_page_down'
        if not self.Success:
            rep.cancel(f)
            return
        self.pos += self.buffer
    ...
    def go_page_up(self):
        f = '[MClient] sources.multitrandem.utils.CompareBinaries.go_page_up'
        if not self.Success:
            rep.cancel(f)
            return
        self.pos -= self.buffer
    ...
    def load(self):
        f = '[MClient] sources.multitrandem.utils.CompareBinaries.load'
        if not self.Success:
            rep.cancel(f)
            return
        start = self.pos
    ...
    def set_buffer(self):
        f = '[MClient] sources.multitrandem.utils.CompareBinaries.set_buffer'
        if not self.Success:
            rep.cancel(f)
            return
        mes = _('Enter a buffer size or press Return to keep the current one ({}): ')
    ...
    def show_help(self):
        f = '[MClient] sources.multitrandem.utils.CompareBinaries.show_help'
        if not self.Success:
            rep.cancel(f)
            return
        mes = _('Available commands: {}')
    ...
    def show_menu(self, command=''):
        f = '[MClient] sources.multitrandem.utils.CompareBinaries.show_menu'
        if not self.Success:
            rep.cancel(f)
            return
        if not command:
    ...
    def quit(self):
        f = '[MClient] sources.multitrandem.utils.CompareBinaries.quit'
        if not self.Success:
            rep.cancel(f)
            return
        self.close()
    ...
    def is_changed(self, i):
        f = '[MClient] sources.multitrandem.utils.CompareBinaries.is_changed'
        if not self.Success:
            rep.cancel(f)
            return
        if i in self.poses:
    ...
    def print(self):
        f = '[MClient] sources.multitrandem.utils.CompareBinaries.print'
        if not self.Success:
            rep.cancel(f)
            return
        mes = _('Chunk 1:')
    ...
    def report(self):
        f = '[MClient] sources.multitrandem.utils.CompareBinaries.report'
        if not self.Success:
            rep.cancel(f)
            return
        self.clear()
    ...
    def compare(self, start=0, end=400):
        f = '[MClient] sources.multitrandem.utils.CompareBinaries.compare'
        if not self.Success:
            rep.cancel(f)
            return
        self.poses = []
    ...
    def close(self):
        f = '[MClient] sources.multitrandem.utils.CompareBinaries.close'
        if not self.Success:
            rep.cancel(f)
            return
        self.bin1.close()
