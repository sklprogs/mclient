...
class Elems:
    ...
    def _get_pair(self, text):
        f = '[MClient] sources.multitrandem.elems.Elems._get_pair'
    ...
    def set_dic_titles(self):
        f = '[MClient] sources.multitrandem.elems.Elems.set_dic_titles'
        if not self.abbr:
            rep.empty(f)
            return
    ...
    def set_art_subj(self):
        f = '[MClient] sources.multitrandem.elems.Elems.set_art_subj'
        count = 0
        for block in self.blocks:
    ...
    def remove_dupl_wforms(self):
        f = '[MClient] sources.multitrandem.elems.Elems.remove_dupl_wforms'
    ...
    def set_source(self):
        f = '[MClient] sources.multitrandem.elems.Elems.set_source'
        if not self.Success:
            rep.cancel(f)
            return
        for block in self.blocks:
    ...
    def run(self):
        f = '[MClient] sources.multitrandem.elems.Elems.run'
        if not self.Success:
            rep.cancel(f)
            return self.blocks
        # Do some cleanup
    ...
    def debug(self, maxrow=20, maxrows=0):
        f = 'sources.multitrandem.elems.Elems.debug'
