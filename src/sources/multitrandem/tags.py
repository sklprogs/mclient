...
class Tags:
    ...
    def get_types(self):
        f = '[MClient] sources.multitrandem.tags.Tags.get_types'
        if not self.Success:
            rep.cancel(f)
            return
        if not self.tags:
    ...
    def set_types(self):
        f = '[MClient] sources.multitrandem.tags.Tags.set_types'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def debug(self):
        f = '[MClient] sources.multitrandem.tags.Tags.debug'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def _debug_tags(self):
        f = '[MClient] sources.multitrandem.tags.Tags._debug_tags'
        mes = []
        for i in range(len(self.tags)):
    ...
    def decode(self):
        f = '[MClient] sources.multitrandem.tags.Tags.decode'
        if not self.Success:
            rep.cancel(f)
            return
        i = 1
    ...
    def set_seps(self):
        f = '[MClient] sources.multitrandem.tags.Tags.set_seps'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def split(self):
        f = '[MClient] sources.multitrandem.tags.Tags.split'
        if not self.Success:
            rep.cancel(f)
            return
        tmp = b''
    ...
    def set_langs(self):
        f = '[MClient] sources.multitrandem.tags.Tags.set_langs'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def check(self):
        f = '[MClient] sources.multitrandem.tags.Tags.check'
    ...
    def set_cellnos(self):
        f = '[MClient] sources.multitrandem.tags.Tags.set_cellnos'
        if not self.Success:
            rep.cancel(f)
            return
