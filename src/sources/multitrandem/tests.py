...
class Ending(gt.Ending):
    ...
    def debug(self):
        f = '[MClient] sources.multitrandem.tests.Ending.debug'
        if not self.Success:
            rep.cancel(f)
            return
        ends = list(self.ends)
...
class Subject(gt.Subject):
    ...
    def debug(self):
        f = '[MClient] sources.multitrandem.tests.Subject.debug'
        if not self.Success:
            rep.cancel(f)
            return
...
class Binary(gt.Binary):
    ...
    def get_max_limits(self, page_no):
        ...
        '''
        f = '[MClient] sources.multitrandem.tests.Binary.get_max_limits'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def show_info(self):
        f = '[MClient] sources.multitrandem.tests.Binary.show_info'
        self.get_block_size()
        self.get_file_size()
        self.get_pages()
        if not self.Success:
    ...
    def get_pages(self):
        f = '[MClient] sources.multitrandem.tests.Binary.get_pages'
        self.get_block_size()
        if not self.Success:
            rep.cancel(f)
            return self.pages
...
class Tests:
    ...
    def get_upage_stems(self):
        f = '[MClient] sources.multitrandem.tests.Tests.get_upage_stems'
    ...
    def get_upage_glue(self):
        f = '[MClient] sources.multitrandem.tests.Tests.get_upage_glue'
        upage = UPage(gt.objs.get_files().iwalker.get_glue1())
        upage.get_parts()
        part1 = list(upage.part1)
        part2 = list(upage.part2)
    ...
    def searchu_stems(self):
        f = '[MClient] sources.multitrandem.tests.Tests.searchu_stems'
        timer = Timer(f)
        timer.start()
        #pattern = b'wol'
        #pattern = b'zero'
    ...
    def translate_many(self):
        f = '[MClient] sources.multitrandem.tests.Tests.translate_many'
        ''' MISSING IN MT DEMO:
    ...
    def translate(self, pattern):
        f = '[MClient] sources.multitrandem.tests.Tests.translate'
        timer = Timer(f)
        timer.start()
...
class UPage(gt.UPage):
    ...
    def debug(self):
        f = '[MClient] sources.multitrandem.tests.UPage.debug'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def debug_glue(self):
        f = '[MClient] sources.multitrandem.tests.UPage.debug_glue'
        if not self.Success:
            rep.cancel(f)
            return
        if not self.part2:
    ...
    def debug_stems(self):
        f = '[MClient] sources.multitrandem.tests.UPage.debug_stems'
        if not self.Success:
            rep.cancel(f)
            return
        if not self.part2:
...
if __name__ == '__main__':
    f = '[MClient] sources.multitrandem.tests.__main__'
