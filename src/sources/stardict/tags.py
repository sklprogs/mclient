...
class AnalyzeTag:
    ...
    def check(self):
        f = '[MClient] sources.stardict.tags.AnalyzeTag.check'
        if not self.fragm:
            self.Success = False
            rep.empty(f)
    ...
    def set_attr(self):
        f = '[MClient] sources.stardict.tags.AnalyzeTag.set_attr'
        if not self.Success:
            rep.cancel(f)
            return
        if self._is_tag():
...
class Tags:
    ...
    def set_inherent(self):
        f = '[MClient] sources.stardict.tags.Tags.set_inherent'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def set_nos(self):
        f = '[MClient] sources.stardict.tags.Tags.set_nos'
        if not self.Success:
            rep.cancel(f)
            return
        curcell = -1
    ...
    def assign(self):
        f = '[MClient] sources.stardict.tags.Tags.assign'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def debug(self):
        f = '[MClient] sources.stardict.tags.Tags.debug'
        if not self.Success:
            rep.cancel(f)
            return ''
        '''
