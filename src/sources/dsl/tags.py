...
class AnalyzeTag:
    ...
    def _set_text(self):
        f = '[MClient] sources.dsl.tags.AnalyzeTag._set_text'
...
class Tags:
    ...
    def set_inherent(self):
        f = '[MClient] sources.dsl.tags.Tags.set_inherent'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def set_nos(self):
        f = '[MClient] sources.dsl.tags.Tags.set_nos'
        if not self.Success:
            rep.cancel(f)
            return
        curcell = -1
    ...
    def assign(self):
        f = '[MClient] sources.dsl.tags.Tags.assign'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def debug(self):
        f = '[MClient] sources.dsl.tags.Tags.debug'
        if not self.Success:
            rep.cancel(f)
            return ''
        '''
