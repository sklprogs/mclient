...
class Get:
    ...
    def _debug_articles(self):
        f = '[MClient] sources.dsl.get.Get._debug_articles'
        mes = [f'{f}:']
    ...
    def debug(self):
        f = '[MClient] sources.dsl.get.Get.debug'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def check(self):
        f = '[MClient] sources.dsl.get.Get.check'
        if not self.pattern:
            self.pattern = ''
    ...
    def search(self):
        f = '[MClient] sources.dsl.get.Get.search'
        if not self.Success:
            rep.cancel(f)
            return
...
class AllDics:
    ...
    def get_langs1(self):
        f = '[MClient] sources.dsl.get.AllDics.get_langs1'
        if not self.Success:
            rep.cancel(f)
            return self.langs1
        if not self.langs1:
    ...
    def get_pairs(self, lang):
        # Both language code and localization name are accepted at input
        f = '[MClient] sources.dsl.get.AllDics.get_pairs'
        pairs = []
        if not self.Success:
            rep.cancel(f)
            return pairs
    ...
    def get_index(self):
        f = '[MClient] sources.dsl.get.AllDics.get_index'
        if not self.Success:
            rep.cancel(f)
            return
        if not self.index_:
    ...
    def walk(self):
        f = '[MClient] sources.dsl.get.AllDics.walk'
        if not self.Success:
            rep.cancel(f)
            return self.dsls
        if self.dsls:
    ...
    def locate(self):
        f = '[MClient] sources.dsl.get.AllDics.locate'
        if not self.Success:
            rep.cancel(f)
            return
        if not self.dics:
