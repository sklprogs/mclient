...
class Dic:
    ...
    def load(self):
        f = '[MClient] sources.fora.get.Dic.load'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def close(self):
        f = '[MClient] sources.fora.get.Dic.close'
        if not self.Success:
            rep.cancel(f)
            return
        self.imap.flush()
    ...
    def get(self, indexes):
        f = '[MClient] sources.fora.get.Dic.get'
        if not self.Success:
            rep.cancel(f)
            return
        if not indexes:
    ...
    def set_file(self):
        f = '[MClient] sources.fora.get.Dic.set_file'
        if not self.Success:
            rep.cancel(f)
            return
...
class Index:
    ...
    def load(self):
        f = '[MClient] sources.fora.get.Index.load'
        if not self.Success:
            rep.cancel(f)
            return
        # .index can be opened as a text file, but no need to read it in full
    ...
    def close(self):
        f = '[MClient] sources.fora.get.Index.close'
        if not self.Success:
            rep.cancel(f)
            return
        self.imap.flush()
    ...
    def search_all(self, pattern):
        f = '[MClient] sources.fora.get.Index.search_all'
        if not self.Success:
            rep.cancel(f)
            return
        bpattern = bytes(pattern, 'utf-8')
    ...
    def set_file(self):
        f = '[MClient] sources.fora.get.Index.set_file'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def decode(self, index):
        ...
        '''
        f = '[MClient] sources.fora.get.Index.decode'
        if not self.Success:
            rep.cancel(f)
            return
        if not index:
...
class Properties:
    ...
    def check_format(self):
        f = '[MClient] sources.fora.get.Properties.check_format'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def set_file(self):
        f = '[MClient] sources.fora.get.Properties.set_file'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def load(self):
        f = '[MClient] sources.fora.get.Properties.load'
        if not self.Success:
            rep.cancel(f)
            return
        try:
    ...
    def set_attrs(self):
        f = '[MClient] sources.fora.get.Properties.set_attrs'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def debug(self):
        f = '[MClient] sources.fora.get.Properties.debug'
        if not self.Success:
            rep.cancel(f)
            return
        mes = []
...
class Fora:
    ...
    def search(self, pattern):
        f = '[MClient] sources.fora.get.Fora.search'
        if not self.Success:
            rep.cancel(f)
            return
        self.article = ''
...
class AllDics:
    ...
    def get_summary(self):
        f = '[MClient] sources.fora.get.AllDics.get_summary'
        if not self.Success:
            rep.cancel(f)
            return
        format_dic = {}
    ...
    def set(self):
        f = '[MClient] sources.fora.get.AllDics.set'
        if not self.Success:
            rep.cancel(f)
            return
        if not self.path:
