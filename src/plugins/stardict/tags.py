...
class AnalyzeTag:
    ...
    def check(self):
        f = '[MClient] plugins.stardict.tags.AnalyzeTag.check'
        if not self.fragm:
            self.Success = False
            rep.empty(f)
    ...
    def _set_text(self):
        f = '[MClient] plugins.stardict.tags.Tags._set_text'
    ...
    def set_attr(self):
        f = '[MClient] plugins.stardict.tags.AnalyzeTag.set_attr'
        if not self.Success:
            rep.cancel(f)
            return
        if self._is_tag():
...
class Tags:
    ...
    def set_inherent(self):
        f = '[MClient] plugins.stardict.tags.Tags.set_inherent'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def set_nos(self):
        f = '[MClient] plugins.stardict.tags.Tags.set_nos'
        if not self.Success:
            rep.cancel(f)
            return
        curcell = -1
    ...
    def set_blocks(self):
        f = '[MClient] plugins.stardict.tags.Tags.set_blocks'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def assign(self):
        f = '[MClient] plugins.stardict.tags.Tags.assign'
        if not self.Success:
            rep.cancel(f)
            return
    ...
    def debug(self):
        f = '[MClient] plugins.stardict.tags.Tags.debug'
        if not self.Success:
            rep.cancel(f)
            return ''
        '''
    ...
    def check(self):
        f = '[MClient] plugins.stardict.tags.Tags.check'
        if not self.code:
    ...
    def split(self):
        f = '[MClient] plugins.stardict.tags.Tags.split'
        if not self.Success:
            rep.cancel(f)
            return
        fragm = ''
