    ...
    def split(self):
        ...
        for i in range(len(self.text)):
            if self.text[i] == '_':
                ...
                ''' Split text only when a short dictionary title is met,
                    ignore the dot otherwise (especially useful when "smb.",
                    "smb.'s", "smth.", "т.п." are met).
                '''
                block += self.text[i]
    ...
    def get_tags(self):
        for block in self.blocks:
            if '<' in block or '>' in block:
                ...
                ''' Examples are essentially comments, but we draft an original
                    as a term because we have both  an original and
                    a translation, and the translation is parsed as a term.
                '''
                self.tags.append('<dtrn>' + block + '</dtrn>')
