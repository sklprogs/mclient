(в разработке)
* Удаление дублирующихся ячеек

## 8.1
* Ускорена загрузка статей
* Проверка видимости ячейки стала более быстрой и точной
* Произведен рефакторинг кода, удалена промежуточная структура cells

## 8.0.2
* Убрано бесполезное предупреждение, когда ничего не было найдено на
  multitran.com

## 8.0.1
* Исправлены падения при выключенном счетчике фраз
* Исправлена сортировка словоформ

## 8.0
* Улучшена поддержка офлайн-словарей
* Поддержка словарей Fora (stardictx, DSL)
* Реализован формат словаря MClient (.mdic)
* Поиск теперь осуществляется по всем источникам сразу
* Добавлен новый режим &mdash; Полный
* Добавлено 2 новых столбца &mdash; Источники и Словари. Источники можно
  приоритизировать и блокировать.
* В главном окне теперь показывается число словарей, которые загрузить
  не удалось
* Добавлен режим конвертации словарей. Для этого запустите программу
  с параметром --convert (поддерживаются DSL, StarDict, Fora (stardictx, DSL)).
